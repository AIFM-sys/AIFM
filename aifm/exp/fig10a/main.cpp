...
class FarMemTest {
private:
  constexpr static uint64_t kCacheSize = 5000 * Region::kSize;
  ...
  Cnt miss_cnts[kNumMutatorThreads];
  std::vector<double> mops_vec;
  std::vector<double> miss_rate_vec;

  uint64_t prev_sum_cnts = 0;
  uint64_t prev_sum_misses = 0;
  uint64_t prev_us = 0;
  ...
  void prepare(GenericConcurrentHopscotch *hopscotch_ptr) {
	...
	DerefScope scope;
	    ...
	    scope.renew();
          ...
          hopscotch_ptr->put(scope, kKeyLen,
    ...
    if (!flag.test_and_set()) {
      ...
      if (us - prev_us > kMinMonitorIntervalUs) {
        ...
        for (uint32_t i = 0; i < kNumMutatorThreads; i++) {
          ...
          sum_misses += ACCESS_ONCE(miss_cnts[i].c);
        }
        us = microtime();
        auto mops = (double)(sum_cnts - prev_sum_cnts) / (us - prev_us);
        auto miss_rate =
            (double)(sum_misses - prev_sum_misses) / (sum_cnts - prev_sum_cnts);
        mops_vec.push_back(mops);
	miss_rate_vec.push_back(miss_rate);
        running_us += (us - prev_us);
        if (running_us >= kMaxRunningUs) {
          std::cout << "mops = " << avg_of_last_N(mops_vec, 2) << ", "
                    << "miss_rate = " << avg_of_last_N(miss_rate_vec, 2)
                    << std::endl;
    ...
    for (uint32_t tid = 0; tid < kNumMutatorThreads; tid++) {
      threads.emplace_back(rt::Thread([&, tid]() {
	    ...
	    scope.renew();
          ...
          hopscotch_ptr->_get(kKeyLen, (const uint8_t *)key.data, &val_len,
                              (uint8_t *)val.data, &forwarded);
          ACCESS_ONCE(cnts[tid].c)++;
          ACCESS_ONCE(miss_cnts[tid].c) += forwarded;
          DONT_OPTIMIZE(val);
        }
      }));
    }
