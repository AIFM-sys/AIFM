...
#include "../../../../../../far_mem_storage/inc/x264.h"
...
static uint8_t buffer[BUFFER_SIZE] __attribute__ ((aligned(4096)));
...
static ssize_t fake_fread(void *dbuf, size_t nbytes)
{
    ...
    c_fm_to_dram(dbuf, seekspot, tocopy);
    seekspot += tocopy;
    return tocopy;
}
...
int open_file_y4m( char *psz_filename, hnd_t *p_handle, x264_param_t *p_param )
{
    ...
    if (!already_loaded) {
        ...
        fh = open(psz_filename, O_RDONLY | O_DIRECT);
        if (fh == -1) {
                return -1;
        }
	c_init_fm();
        long long sum = 0, cur, tmp;
        fprintf(stderr, "File size = %ld bytes.\n", fullbuffer_size);
        while (sum != fullbuffer_size) {
	  cur = 0;
          while (cur < BUFFER_SIZE) {
            tmp = read(fh, buffer + cur, BUFFER_SIZE - cur);
            fprintf(stderr, "tmp = %ld bytes.\n", tmp);
            if (tmp <= 0) {
              break;
            }
            cur += tmp;
          }
          fprintf(stderr, "cur = %ld bytes.\n", cur);
          c_dram_to_fm(buffer, sum, cur);
          sum += cur;
          fprintf(stderr, "Have read %lld bytes.\n", sum);	  
        }
        if (sum != fullbuffer_size) {
                fprintf(stderr, "Could not preload buffer %d.\n", ferror(fh));
                return -1;
        }
...
int get_frame_total_y4m( hnd_t handle )
{
    ...
    i_frame_total =
        (int)((fullbuffer_size - h->seq_header_len) /
