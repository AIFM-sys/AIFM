...
int open_file_y4m( char *psz_filename, hnd_t *p_handle, x264_param_t *p_param )
{
    ...
    if (!already_loaded) {
        ...
        fullbuffer = numa_alloc_onnode(fullbuffer_size + 512, 1);
        if (!fullbuffer) {
                return -1;
        }
        memset(fullbuffer, 0, fullbuffer_size);
        fh = open(psz_filename, O_RDONLY | O_DIRECT);
        if (fh == -1) {
                return -1;
        }
        long long tmp, sum = 0;
        fprintf(stderr, "File size = %ld bytes.\n", ((fullbuffer_size - 1) / 512 + 1) * 512);
        while ((tmp = read(fh, fullbuffer + sum, 1024 * 1024 * 128)) > 0) {
                sum += tmp;
